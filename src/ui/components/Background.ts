...
export class Background extends Phaser.GameObjects.Sprite {
  ...
  protected create() {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      star1.x = Math.random() * screenWidth;
      star1.y = Math.random() * screenHeight;
      ...
      star2.x = Math.random() * screenWidth;
      star2.y = Math.random() * screenHeight;
  ...
  public update() {
    ...
    this.starsLayer1.children.iterate((star: Phaser.GameObjects.GameObject) => {
      ...
      starGraphics.y += 0.5;
      if (starGraphics.y > screenHeight) {
        starGraphics.y = 0;
        starGraphics.x = Math.random() * screenWidth;
      }
